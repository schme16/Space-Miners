...
function TEsound.volume(channel, volume)
	if type(channel) == "number" then
		...
		c[1]:setVolume( volume * TEsound.findVolume(c.tags) * (TEsound.pitchLevels.all or 1) )
	elseif type(channel) == "string" then for k,v in pairs(TEsound.findTag(channel)) do TEsound.volume(v, volume) end
	end
end
...
function TEsound.pitch(channel, pitch)
	...
	elseif type(channel) == "string" then for k,v in pairs(TEsound.findTag(channel)) do TEsound.pitch(v, pitch) end
	end
end
...
function TEsound.stop(channel, finish)
	if type(channel) == "number" then
		local c = TEsound.channels[channel] c[2] = nil if not finish then c[1]:pause() table.remove(TEsound.channels, channel) end
